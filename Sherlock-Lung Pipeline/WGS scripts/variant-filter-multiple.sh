...
python /home/zhangt8/bin/add_to_vcf_header.py combine_snv_filter.vcf '##INFO=<ID=OVERLAP,Number=0,Type=Flag,Description="Somatic indel possibly overlaps a second indel.">' >combine_snv_filter.vcf2
...
## for genomad v4
table_annovar.pl all2.avinput $ANNOVAR_DATA/hg38 --buildver hg38 --outfile all2 --remove --protocol refGene,ensGene,ljb26_all,dbnsfp35c,dbscsnv11,cosmic92_coding,cosmic92_noncoding,1000g2015aug_all,avsnp150,clinvar_20200419,gnomad40  --operation g,g,f,f,f,f,f,f,f,f,f --otherinfo  --nastring NA 
head -1  all2.hg38_multianno.txt |transpose -t|nl|awk '$2=="gnomad40_AF"{print $1}END{print $1}' |paste -s -d "," |rush ' cut -f {}  all2.hg38_multianno.txt' |awk -F "\t" -v OFS="\t"  'NR>1 && ($1!="NA" && $1>0.001) {print $NF}' >MAF_filter.txt 

zcat ${PREFIX}.vcf.gz  >tmp.vcf
bcftools view -H all2.vcf|awk -F "\t" -v OFS="\t" 'NR==FNR{a[$1]=1;next;}($3 in a){ print $0}' MAF_filter.txt - |awk -F "\t" -v OFS="\t" 'NR==FNR{a[$1$2$4$5]="MAF001";next;}{if(!/^#/) {key=$1$2$4$5; if(key in a) {if($7=="") $7=a[key];else $7=$7";"a[key]; gsub("PASS;","",$7);}}; print $0;}' - tmp.vcf |sed  '/^##FILTER=<ID=PASS/a ##FILTER=<ID=MAF001,Description="filter by MAF <0.001 in gnomADv4.0">' >${PREFIX}.filter3.vcf 
