...
tdata0 <- covdata0 %>% 
  ...
  select(Tumor_Barcode,Histology,Assigned_Population,Gender,Smoking,Passive_Smoking) %>% 
  #mutate(Smoking_Signature=SBS4) %>% 
  filter(Smoking=='Non-Smoker') 
#%>%  mutate(Histology=if_else(Histology == 'Other','Others',Histology))

tdata0 %>% count(Histology,Assigned_Population,Gender,Passive_Smoking) %>% View()

tdata <- tdata0 %>% make_long(Histology,Assigned_Population,Gender,Passive_Smoking)
tdata_nr <- 
  tdata %>% 
  filter(!is.na(node)) %>% 
  group_by(x, node)%>% 
...
tdata$node <- factor(tdata$node,levels = c('Adenocarcinoma','Squamous cell carcinoma','Carcinoid tumor','Others','EUR','EAS','Other','Female','Male','Yes','No','Unknown'))

ggplot(tdata, aes(x = x, next_x = next_x, node = node, next_node = next_node, fill = factor(node), label = node)) +
  geom_sankey(flow.alpha = .6, node.color = "gray30",space = 30) +
...
ggsave(filename = 'Sherlock_871_WGS_dataset.pdf',width = 8,height = 8,device = cairo_pdf)
...
pall1 <- plot_grid(p1,NULL,p2,align = 'h',nrow = 1,axis = 'tb',rel_widths = c(3,-0.33,1.5))
pall2 <- plot_grid(p3,NULL,p4,align = 'h',nrow = 1,axis = 'tb',rel_widths = c(3,-0.33,1.5))
pall3 <- plot_grid(p5,NULL,p6,align = 'h',nrow = 1,axis = 'tb',rel_widths = c(3,-0.33,1.5))
...
tdata <- bind_rows(
  data1$freq_data %>% mutate(Group='EAS'),
  data2$freq_data %>% mutate(Group='EUR')
)
...
plotdata <- tdata %>% 
  ...
  mutate(Value = if_else(Group=='EAS',-1*Value,Value)) %>% 
  mutate(Value2 = if_else(Group=='EAS',-1*Value2,Value2)) 
...
p1 <- plotdata %>% 
  ...
  geom_text(data = plotdata %>% filter(Group=='EAS'),aes(label=Percentage),vjust=0.5,hjust=1.2)+
  geom_text(data = plotdata %>% filter(Group=='EUR'),aes(label=Percentage),vjust=0.5,hjust=-0.2)+
...
p2 <- testdata %>% 
  ...
  scale_fill_manual(values = sigcol)+
  scale_x_continuous(breaks = pretty_breaks(n = 7),limits = c(-4.05,4.05),position = "top")+
...
ggsave(filename = 'LUAD_SBS_Ancestry_prevalence_tmp.pdf',width = 6,height = 6,device = cairo_pdf)
...
ggsave(filename = 'LUAD_N_driver_ancestry.pdf',width = 5,height = 4,device = cairo_pdf)

plotdata <- testdata %>% 
  filter(Gene %in% c('EGFR','KRAS','TP53')) %>% 
  count(Gene,Assigned_Population,Alteration) %>% 
  group_by(Gene,Assigned_Population) %>% 
  mutate(freq=n/(sum(n))) %>% 
...
plotdata %>% 
  ...
  ggplot(aes(Gene,freq,group=Assigned_Population,fill=Assigned_Population))+
  ...
  panel_border(color = 'black',size = 0.5)
ggsave(filename = 'LUAD_N_driver_ancestry2.pdf',width = 3,height = 4,device = cairo_pdf)
...
testdata %>% 
  ...
  theme_ipsum_rc(base_size = 12,axis_title_just = 'm',axis_title_size = 12,grid = F,ticks = T,plot_margin=margin(5.5,5.5,5.5,5.5))+
  theme(plot.title = element_text(hjust = 0.5))+
  labs(x = 'Odd ratio (log2)', y = '-log10(FDR)',title='SBS mutational sigantures')+
...
stat.test <- plotdata %>%
  left_join(covdata0) %>% 
  group_by(Type) %>% 
  do(tidy(lm(Contribution ~ Passive_Smoking + Age + Gender + Assigned_Population + Histology + Tumor_Purity, data=.),conf.int = TRUE, conf.level = 0.95)) %>% 
  filter(term=='Passive_SmokingYes') %>% 
  mutate(FDR=p.adjust(p.value,method='BH')) %>% 
...
plotdata <- testdata %>% 
  group_by(Gene) %>% 
  mutate(Alteration=as.factor(Alteration)) %>% 
  do(tresult = safely(glm)(Alteration ~ Passive_Smoking + Histology + Assigned_Population+ Gender + Age + Tumor_Purity, family='binomial',data=. )) %>% 
  mutate(tresult_null = map_lgl(tresult['result'], is.null)) %>% 
  filter(!tresult_null) %>% 
  mutate(fit = list(tidy(tresult[['result']],exponentiate=TRUE))) %>% 
  select(Gene,fit) %>% 
...
plotdata <- tdata %>% select(Tumor_Barcode,Telseq_TL_Ratio,Pollution_group2) %>%  filter(!is.na(Telseq_TL_Ratio)) %>%  left_join(covdata0)

plotdata <- plotdata %>% 
  do(tidy(lm(Telseq_TL_Ratio ~ Pollution_group2 + Age + Gender + Assigned_Population + Histology + Tumor_Purity, data=.),conf.int = TRUE, conf.level = 0.95)) %>% 
  filter(term != '(Intercept)') %>% 
  #mutate(FDR=p.adjust(p.value,method='BH')) %>% 
  mutate(label=paste0('β = ',round(estimate,2), ', q-value = ',scientific_format(digits = 3)(p.value))) %>% 
  arrange(term) %>% 
...
plotdata <- testdata %>% 
  group_by(Gene) %>% 
  mutate(Alteration=as.factor(Alteration)) %>% 
  do(tresult = safely(glm)(Alteration ~ PM25 + Histology + Assigned_Population+ Gender + Age + Tumor_Purity, family='binomial',data=. )) %>% 
  mutate(tresult_null = map_lgl(tresult['result'], is.null)) %>% 
  filter(!tresult_null) %>% 
  mutate(fit = list(tidy(tresult[['result']],exponentiate=TRUE))) %>% 
  select(Gene,fit) %>% 
...
barplot_fisher(plotdata %>% filter(Gene=='TP53'),'Pollution_group2','Alteration',var1lab =  expression("Pollution group (Population weighted PM"[2.5]*")"),var2lab = 'Mutation status',filename = 'LCINS_Pollution_driver_TP53.pdf')

barplot_fisher(plotdata %>% filter(Gene=='CTNNB1'),'Pollution_group2','Alteration',var1lab =  expression("Pollution group (Population weighted PM"[2.5]*")"),var2lab = 'Mutation status',filename = 'LCINS_Pollution_driver_CTNNB1.pdf')
